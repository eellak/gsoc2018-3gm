...
def test_phrase():
    ...
    test1 = phrase_fun.replace_phrase(
        test, 'επίδοσής της στο αντίδικο μέρος', 'foo')
    tmp = '. '.join(test1)
    ...
    test5 = phrase_fun.insert_phrase(
        test1, 'boo', position='before', old_phrase='foo')
    tmp = '. '.join(test5)
    assert('boo foo' in tmp)

    test6 = phrase_fun.insert_phrase(
        test1, 'boo', position='after', old_phrase='foo')
    tmp = '. '.join(test6)
    assert('foo boo' in tmp)

    test7 = law.insert_phrase(
        'boo', position='before', old_phrase='επίδοσής της στο αντίδικο μέρος', article='1', paragraph='1')
    assert('boo επίδοσής της στο αντίδικο μέρος' in '. '.join(
        test7['articles']['1']['1']))
...
def test_phrase_ops():
    ...
    tr = {'phrase': {'old_phrase': 'κατά το χρόνο που θα οριστούν τα ανωτέρω σχολεία ως', 'new_phrase': 'στα νέα σχολεία'}, 'what': {
        'content': 'κατά το χρόνο που θα οριστούν τα ανωτέρω σχολεία ως', 'number': ['phrase']}, 'root': {'action': 'αντικαθίσταται'}}
    qr = {'phrase': {'location': 'after', 'new_phrase': ', υπό τις προϋποθέσεις και τις διαδικασίες που ορίζονται στην παράγραφο 4,', 'old_phrase': 'Ο Μουφτής ασκεί δικαιοδοσία'},
          'what': {'number': ['phrase'], 'content': ', υπό τις προϋποθέσεις και τις διαδικασίες που ορίζονται στην παράγραφο 4,'}, 'root': {'action': 'προστίθεται'}}
    rr = {'phrase': {'location': 'append', 'new_phrase': 'και τις υποχρεώσεις για την υποβολή πληροφοριών κατάπλου και απόπλου, που απορρέουν από την ενωσιακή νομοθεσία και περιγράφονται στο π.δ. 49/2005 (Α΄ 66), όπως έχει τροποποιηθεί και ισχύει, και στο π.δ. 16/2011 (Α΄ 36).', 'old_phrase': ''}, 'what': {
        'number': ['phrase'], 'content': 'και τις υποχρεώσεις για την υποβολή πληροφοριών κατάπλου και απόπλου, που απορρέουν από την ενωσιακή νομοθεσία και περιγράφονται στο π.δ. 49/2005 (Α΄ 66), όπως έχει τροποποιηθεί και ισχύει, και στο π.δ. 16/2011 (Α΄ 36).'}, 'root': {'action': 'προστίθεται'}}
    sr = {'phrase': {'old_phrase': 'και το ισχύον ποσοστό κάλυψης'}, 'what': {'number': [
        'phrase'], 'content': 'και το ισχύον ποσοστό κάλυψης'}, 'root': {'action': 'διαγράφεται'}}

    assert(phrase_fun.detect_phrase_components(
        t,  {'root': {'action': 'αντικαθίσταται'}, 'what': {},  'phrase': {}}) == tr)
    assert(phrase_fun.detect_phrase_components(
        q,  {'root': {'action': 'προστίθεται'}, 'what': {},  'phrase': {}}) == qr)
    assert(phrase_fun.detect_phrase_components(
        r,  {'root': {'action': 'προστίθεται'}, 'what': {},  'phrase': {}}) == rr)
    assert(phrase_fun.detect_phrase_components(
        s,  {'root': {'action': 'διαγράφεται'}, 'what': {},  'phrase': {}}) == sr)

# Tokenization test


def test_tokenizer():
    assert(tokenizer.tokenizer.split('Έλα στις 6 π.μ. και μην αργήσεις. Είναι σημαντικό',
                                     False, '.') == ['Έλα στις 6 π.μ. και μην αργήσεις', ' Είναι σημαντικό'])
    assert(tokenizer.tokenizer.remove_subordinate(
        'Κάτι, όπως έγινε, δεν είναι καλό') == 'Κάτι δεν είναι καλό')
