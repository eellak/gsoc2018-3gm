...
# Caching costants
CACHE_ENABLED = True
CACHE_DEFAULT_EXPIRE_TIME = 86400 # for redis cache set default expire time to 24 hours
...
def cache_store(cache_key,val,expires=True, compress=False):
    ...
    if not isinstance(val,str):
        val = dumps(val,json_options=json_util.RELAXED_JSON_OPTIONS,ensure_ascii=False)
...
def get_topics_for_statute(statute_id):
    l = list((x['keywords'] for x in codifier.topics if statute_id in  x['statutes'])) 
    s = ' '.join(np.unique(np.array(l).flatten()))
    docx = nlp(s)
    e = list(set(token.lemma_ for token in docx if len(token)>3 and token.is_stop != True and token.is_punct != True and token.pos_ == 'NOUN'))
    ...
    for item in list(codifier.keys()):
        try:
            ...
            if year and year<= datetime.today().year and name>0:
                legal_index.append({'_id': id , 'name':name , 'type':t , 'year':year, 'rank':rank})
        except:
            pass
...
class IaStatsResource(Resource):
    def get(self):
        ...
        res_count_lastyear = db.ia.count_documents({'addeddate':{'$lt': datetime.now(), '$gte': datetime.now() - timedelta(days=365)}})
        res_count_lastmonth = db.ia.count_documents({'addeddate':{'$lt': datetime.now(), '$gte': datetime.now() - timedelta(days=30)}})
        res_count_lastweek = db.ia.count_documents({'addeddate':{'$lt': datetime.now(), '$gte': datetime.now() - timedelta(days=7)}})
        res_last_docs = list(db.ia.find({} ,{'identifier':1,'title':1,'addeddate':1}).sort('addeddate', -1).limit(5))

        res = {'res_count_total':res_count_total
        ,'res_count_lastyear':res_count_lastyear
        ,'res_count_lastmonth':res_count_lastmonth
        ,'res_count_lastweek':res_count_lastweek
        ,'res_last_docs': res_last_docs}
...
class StatuteStatsResource(Resource):
    def get(self):
        ...
        res = {
        'res_laws_total':num_laws
        ,'res_links_total':num_links
        ,'res_topics_total':num_topics
        ,'last_5_laws': last_5_laws
        ,'last_5_pd':last_5_pd
        ,'iarchive_stats':iarchive_stats
        }
...
class StatuteResource(Resource):
    def get(self,statute_id):
        ...
        archive = codifier.db.archive_links.find_one({'_id' : _id},{'issue':1 , '_id': 0})
        archive_issue = (archive or {}).get('issue','')
        name,year = (int(d) for d in  statute_id.split('_')[1:])
        type = statute_id.split('_')[:1][0]
        res = {'_id':statute_id , 'titleGR': _id , 'rank':rank , 'archive':archive_issue, 'name': name , 'year': year , 'type':type} 
        cache_store(cache_key,res)
        return res
...
class StatuteHistoryResource(Resource):
    def get(self,statute_id):
        ...
        for law in reversed(history):
            ...
            res['issue'] = law.issue
            res['summary'] = codifier.db.summaries.find_one({'_id' : law.amendee},{'summary':1 , '_id': 0})['summary']
            res['archive'] = codifier.db.archive_links.find_one({'_id' : law.amendee},{'issue':1 , '_id': 0})['issue']
            jsonres.append(res)
                    ...
                    if result != []:
                        for found in result:
                            if 'law' in found['root']['children'] and found['law']['_id']:
                                ...
                                item = (found['root']['action'] , found['what']['context'])
                                amendments.setdefault(statute, set([])).add(item)
                except BaseException:
                    continue
...
class StatuteDiffResource(Resource):
    ...
    def get(self,statute_id,amendee_id):
        ...
        version_initial = next((x for x in history if x.amendee==_initial),None)
        ...
        if version_initial and version_final:
            ...
            initial_text =  version_initial.export_law('issue').splitlines(keepends=True)
            final_text = version_final.export_law('issue').splitlines(keepends=True)

            n = 1000 if not args.get('stripcontext', 'false')=='true' else 5
            diffs = difflib.unified_diff(initial_text, final_text , fromfile=_id, tofile=_final , n=n)
...
api.add_resource(StatuteCodifiedResource, '/statute/<string:statute_id>/codified')
api.add_resource(StatuteHistoryResource, '/statute/<string:statute_id>/history')
api.add_resource(StatuteLinksResource, '/statute/<string:statute_id>/links')
api.add_resource(StatuteDiffResource, '/statute/diff/<string:statute_id>/<string:amendee_id>')

api.add_resource(StatuteArticlesResource, '/statute/<string:statute_id>/articles')
