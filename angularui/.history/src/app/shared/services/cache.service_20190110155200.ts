...
export class RequestCacheWithLocalStorage implements RequestCache {
  ...
  get(req: HttpRequest<any>): Observable< HttpResponse<any>> | undefined  {
    ...
    .pipe(
      switchMap((x) => (isNil(x) || x.lastRead < (Date.now() - maxAgeInLocalStorage)) ? of(undefined) :  of(x.response))
    );
  ...
  put(req: HttpRequest<any>, response: HttpResponse<any>): void {
    ...
    if (response.body) {
      ...
      this.memoryput(req, response);
      this.storagecache.setItem(url, entry.lastRead).subscribe(() => {
