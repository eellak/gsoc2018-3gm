...
export class CachingInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler) {
    ...
    const cachedResponse2 =  this.cache.get(req);

  return of(cachedResponse2).pipe(
     tap((value) =>  value ?  of(value) : sendRequest(req, next, this.cache) //{
 ...
 /*   if (req.headers.get('x-refresh')) {
   ...
   //return  value ?  of(value) : sendRequest(req, next, this.cache);

  // }
   ),
   catchError(err => {console.log(err) ;  return sendRequest(req, next, this.cache);} )
  );
  }
}
