...
import { Statute2UrlPipe } from '@app/shared/pipes/statute2url.pipe';
...
export class StatuteReferencesComponent implements OnInit, OnDestroy {
        ...
        .subscribe((eventData: any[]) => {
            if (eventData[0] === this.visNetwork) {
              ...
              if (data && data.length>0) {
                const statute_title = data[0];
                const statute_id = new Statute2UrlPipe().transform(statute_title);
                console.log(statute_id);

                this.statuteID = statute_id;
                this.getLinks();
              }
  ...
  getLinks() {
    ...
    const timer: ReturnType<typeof setTimeout> = setTimeout(() => this.loader.close() , 15000);
  ...
  level++;
    ...
    for (const key in outgoing_nodes) {
      ...
      const tooltip = value.map( val =>  val[0] + ' ' + val[1]).join('</br>');
