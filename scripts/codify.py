    ...
    try:
        ...
        if not os.path.isfile(output_txt):
            if os.path.isfile(input_pdf):
                os.system('python3 {} {} > {}'.format(pdf2txt, input_pdf, output_txt))
            else:
                logging.info('{}: File not found!'.format(input_pdf))
                return False
            if os.stat(output_txt).st_size <= MIN_BYTES:
                logging.info('{}: File Size unsatisfactory. Performing OCR'.format(input_pdf))
                ocr.pdfocr2txt(input_pdf, output_txt, resolution=resolution, tmp=tmp)
        ...
        logging.error("Exception occurred while converting file %s to txt", input_pdf,exc_info=True)
        return False
...
if __name__ == '__main__':
    ...
    optional.add_argument('--pdf2txt', default='pdf2txt' , help='pdf2txt.py Executable')
    ...
    input_data_folder = Path(data_dir_pdf / data_dir_teuxos / data_dir_year / data_dir_subid )
    output_data_folder = Path( data_dir_txt / data_dir_year)
    
    full_path_to_pdf = str(input_data_folder / filename)
    full_path_to_txt = str( output_data_folder / filename.replace('.pdf', '.txt'))
