...
conditions = ['Εκτός αν', 'αν', 'προϋπόθεση', 'κατά περίπτωση' ,'εφόσον' , 'εάν',  'ανεξαρτήτως εάν', 'είναι δυνατόν να' , 'τις προϋποθέσεις', 'μόνο εφόσον' , 'μετά από' , 'ενέχει']

constraints = ['εν όλω', 'εν μέρει', 'αρκεί' , 'εκτός από' , 'πρέπει να' , 'πλην' , 'τουλάχιστον', 'μέχρι', 'το πολύ']

durations = [ 'επί', 'μέσα στον μήνα', 'μέσα σε', 'εντός ', 'μέχρι της ίδιας αυτής ημερομηνίας', 'προθεσμία' ,  'το αργότερο εντός']
...
if __name__ == "__main__":
    ...
    id_numbers = re.findall('Α.Δ.Τ. [Α-Ωα-ω]{0,2} [0-9]{6}|Α.Δ.Τ.: [Α-Ωα-ω]{0,2}-[0-9]{6}|Α.Δ.Τ. [Α-Ωα-ω]{0,2}[0-9]{6}|ΑΔΤ Α[Α-Ωα-ω]{0,2}[0-9]{6}',data)
    print("ID numbers: ",  len(id_numbers))

    #IBAN accounts
    ibans = re.findall('^[A-Z]{2}(?:[ ]?[0-9]){18,20}$|[A-Z]{2} [0-9]{25}',data)
    ...
    phone_numbers = re.findall('τηλ. .+[0-9]|τηλ: .+[0-9]|tel: .+[0-9]|τηλ.: .+[0-9]|Τηλέφωνο: .+[0-9]',data)
    ...
    exact_times = re.findall('[0-9]{2}:[0-9]{2} π.μ.|[0-9]{2}:[0-9]{2} μ.μ.|[0-9]{2}:[0-9]{2}|[0-9]{2}:[0-9]{2}', data)
    print("Exact time: ", len(exact_times))
    
    #money YODD many variations
    money = re.findall(r'[$€£]{1}\d+\.?\d{0,2}|\d+\.\d+\,?\d{0,2} [$€£]{1}|\d+\.\d+\,?\d{0,2} EUR|\d+\.\d+\,?\d{0,2} ευρώ', data)
    print("Monetary amounts: ", len(money))

    #Other issues (simple heurestic needs work finds around half)
    issues = re.findall('ΦΕΚ [0-9]{1,4}[/.-]{0,1}[Α-Ωα-ω.΄\s][/.-]{0,1}.+[0-9]|\(ΦΕΚ [0-9]{1,4}[/.-]{0,1}[Α-Ωα-ω.΄\s][/.-]{0,1}.+[0-9]',data)
    ...
    hull  =  re.findall('HULL No [A-Z0-9 -]{1,17}|HULL No [A-Z]{1,2} [0-9]{1,17}',data)
