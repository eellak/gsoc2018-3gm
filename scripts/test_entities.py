...
if __name__ == "__main__":
    ...
    # Weight
    weight = re.findall('[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d* Kg|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d* kgr|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*kg|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d* kg|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*Kg', data)
    print("Weights: ", len(weight))

    # Volume
    volumes = re.findall('[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)* λίτρα|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* λίτρα|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d*lt|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d* It|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d\d)*[\.]?\d* lt|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* ml', data)
    print("Volumes: ", len(volumes))

    # Ship tonnage in register tons
    tonnage = re.findall(
        '[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* κόροι|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* κόρωv', data)
    print("Tonnage: ", tonnage)

    # Power
    power = re.findall(
        '[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* KW|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)*KW', data)
    print("Power mesurements: ", power)

    # Surface
    surface = re.findall('[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* μ2|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* τετραγωνικών μέτρων|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* τμ|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* τ.μ.|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* στρέμματα|[-+]?[.]?[\d]+[-+]?[.]?[\d]+(?:,\d\d)* στρ', data)
